...
import { addStorage, createFile, listFiles, type DiskWithVmId } from '@oxide/api-mocks'

import {type Disk, type Host, LoginResponse, type Storage, User, type VirtualMachine} from '~/api'
import { json, makeHandlers, type Json } from '~/api/__generated__/msw-handlers'
import { GiB, TiB } from '~/util/units.ts'

import {db, InvalidLoginErr, LoginFailedErr, notFoundErr} from './db'
import { NotImplemented, paginated } from './util.ts'
import type {Promisable} from "type-fest";
import type * as Api from "../../app/api/__generated__/Api.ts";
...
export const handlers = makeHandlers({
  ...
  getStoragesOnHost({ path, query }) {
    const storages = db.storages.filter((storage) => storage.host_id === path.hostId)
