  ...
  configureServer(server) {
    server.middlewares.use((req, res, next) => {
      if (req.url.startsWith('/vnc')) {
        const secondaryHtml = fs.readFileSync(
          resolve(__dirname, './app/pages/vnc/index.html'),
          'utf-8'
        )
        res.statusCode = 200
        res.setHeader('Content-Type', 'text/html')
        res.end(secondaryHtml)
      } else {
        next()
      }
    })
...
export default defineConfig(({ mode }) => ({
  build: {
    ...
    rollupOptions: {
      input: {
        main: 'index.html',
        secondary: resolve(__dirname, './app/pages/vnc/index.html'),
      },
    },
