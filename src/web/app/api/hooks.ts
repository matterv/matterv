...
export const getUseApiQuery =
  ...
  <M extends string & keyof A>(
    ...
    return useQuery({
      ...
      queryFn: ({ signal }) =>
        api[method](params, { signal: signal, host: params?.params?.host }).then(
          handleResult(method)
        ),
...
export const getUseApiQueryErrorsAllowed =
  ...
  <M extends string & keyof A>(
    ...
    return useQuery({
      ...
      queryFn: ({ signal }) =>
        api[method](params, { signal: signal, host: params?.params?.host })
  ...
  <M extends string & keyof A>(
    ...
    useMutation({
      mutationFn: ({ signal, ...params }) =>
        api[method](params, { signal, host: params?.params?.host }).then(
          handleResult(method)
        ),
      // no catch, let unexpected errors bubble up
      ...options,
...
export const wrapQueryClient = <A extends ApiClient>(api: A, queryClient: QueryClient) => ({
  ...
  fetchQuery: <M extends string & keyof A>(
    ...
    queryClient.fetchQuery({
      queryKey: [method, params],
      queryFn: () =>
        api[method](params, { host: params?.params?.host }).then(handleResult(method)),
      ...options,
    }),
  ...
  prefetchQueryErrorsAllowed: <M extends string & keyof A>(
    ...
    queryClient.prefetchQuery({
      queryKey: [method, params, ERRORS_ALLOWED],
      queryFn: () =>
        api[method](params, { host: params?.params?.host })
          .then(handleResult(method))
          .then((data) => ({ type: 'success' as const, data }))
          .catch((data: ApiError) => {
