...
export const INSTANCE_MAX_RAM_MiB = 256 * 1024
...
const instanceActions: Record<string, VMStatus[]> = {
  start: ['STOPPED'],
...
export const instanceCan = R.mapValues(instanceActions, (states) => {
  const test = (i: { status: VMStatus }) => states.includes(i.status)
  test.states = states
  return test
})

export function instanceTransitioning({ status }: VirtualMachine) {
  return (
    status === 'CREATING' ||
    status === 'STOPPING' ||
    status === 'REBOOTING' ||
