...
export function FileListTable({
  ...
  const rows = useMemo(() => (files?.items || []).map((file) => ({ ...file })), [files])
  ...
  useEffect(() => {
    if (table.getSelectedRowModel().rows.length > 0) {
      if (currentDir === '/') {
        onIsoPathChange(currentDir + table.getSelectedRowModel().rows[0].original.name)
      } else {
        onIsoPathChange(
  ...
  return (
    <>
      <TableActions className="!justify-between">
        <CurrentDir currentDir={currentDir} goToRoot={() => setCurrentDir('/')} />
      </TableActions>
      {files?.items.length === 0 ? <EmptyState /> : <Table table={table} singleSelect />}
    </>
  )
}
