
package com.vmware.vim25;

import java.util.ArrayList;
import java.util.List;
import jakarta.xml.bind.annotation.XmlAccessType;
import jakarta.xml.bind.annotation.XmlAccessorType;
import jakarta.xml.bind.annotation.XmlType;


/**
 * <p>Java class for VmwareDistributedVirtualSwitchTrunkVlanSpec complex type</p>.
 * 
 * <p>The following schema fragment specifies the expected content contained within this class.</p>
 * 
 * <pre>{@code
 * <complexType name="VmwareDistributedVirtualSwitchTrunkVlanSpec">
 *   <complexContent>
 *     <extension base="{urn:internalvim25}VmwareDistributedVirtualSwitchVlanSpec">
 *       <sequence>
 *         <element name="vlanId" type="{urn:internalvim25}NumericRange" maxOccurs="unbounded" minOccurs="0"/>
 *       </sequence>
 *     </extension>
 *   </complexContent>
 * </complexType>
 * }</pre>
 * 
 * 
 */
@XmlAccessorType(XmlAccessType.FIELD)
@XmlType(name = "VmwareDistributedVirtualSwitchTrunkVlanSpec", propOrder = {
    "vlanId"
})
public class VmwareDistributedVirtualSwitchTrunkVlanSpec
    extends VmwareDistributedVirtualSwitchVlanSpec
{

    protected List<NumericRange> vlanId;

    /**
     * Gets the value of the vlanId property.
     * 
     * <p>This accessor method returns a reference to the live list,
     * not a snapshot. Therefore any modification you make to the
     * returned list will be present inside the JAXB object.
     * This is why there is not a <CODE>set</CODE> method for the vlanId property.</p>
     * 
     * <p>
     * For example, to add a new item, do as follows:
     * </p>
     * <pre>
     * getVlanId().add(newItem);
     * </pre>
     * 
     * 
     * <p>
     * Objects of the following type(s) are allowed in the list
     * {@link NumericRange }
     * </p>
     * 
     * 
     * @return
     *     The value of the vlanId property.
     */
    public List<NumericRange> getVlanId() {
        if (vlanId == null) {
            vlanId = new ArrayList<>();
        }
        return this.vlanId;
    }

}
